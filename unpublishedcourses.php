...
/**
 * Drill-down list for the "unpublished courses" health signal (step 19).
 *
...
$PAGE->set_url(new \core\url('/local/admincockpit/unpublishedcourses.php'));
$PAGE->set_context($context);
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('unpublishedcourses', 'local_admincockpit'));
$PAGE->set_heading(get_string('pluginname', 'local_admincockpit'));

dashboard_viewed::create([
    'context' => $context,
    'other' => ['page' => 'unpublishedcourses.php'],
])->trigger();

$days = (int) (get_config('local_admincockpit', 'unpublishedcoursedays') ?: 90);
$signal = health_signals::unpublished_courses($days);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('unpublishedcourses', 'local_admincockpit'));
echo $OUTPUT->notification(get_string('unpublishedcourses_intro', 'local_admincockpit', $days), 'info');

if (empty($signal->details)) {
    echo $OUTPUT->notification(get_string('unpublishedcourses_none', 'local_admincockpit'), 'success');
} else {
    if ($signal->detailstruncated) {
        echo $OUTPUT->notification(
            get_string('unpublishedcourses_truncated', 'local_admincockpit', $signal->count),
            'info'
        );
    }
