...
| Health signal extension hook (2026-08-02, released 2.2.0) | `local_admincockpit\hook\health_signals` (`classes/hook/health_signals.php`) - any Moodle plugin can register a `db/hooks.php` callback and add its own tile via `local_admincockpit\health_signal` (`classes/health_signal.php`), no PR or coordination with this plugin needed. The four (now five) built-in signals are this plugin's own consumer of the same hook (`classes/hook/local_listener.php`), not a special case. See README "Extending: add your own health signal" for the third-party-facing docs. |
| Health signal: Unpublished courses (2026-08-02, released 2.2.0) | `course.visible = 0` paired with a configurable age threshold (`local_admincockpit/unpublishedcoursedays`, default 90 days) - deliberately not a bare visibility check, since hiding a course while preparing it is normal, not a problem (same false-positive trap already avoided for the rejected auth-mismatch signal below). Implemented as the extension hook's first third-party-style consumer (a "dogfood" case), proving the mechanism works before documenting it externally. |
| Quick action: One-click impersonate (2026-09-24, released 2.5.0) | Section 5's quick-login, built as a thin wrapper around core rather than a reimplementation: the user search is core's own `core_user/form_user_selector` autocomplete (backed by the existing `core_user_search_identity` web service, so no bespoke search endpoint and no new web service), and the authorisation decision delegates to `\core\session\loginas_helper::get_context_user_can_login_as()` - the same helper core's own `course/loginas.php` uses - before calling `\core\session\manager::loginas()`. Verified in core first: `manager::loginas()` performs **no** capability check of its own, so every guard is the caller's responsibility. On top of core's rules this plugin adds exactly one, per section 5's escalation requirement: a site admin is never a valid target, because core deliberately allows admin→admin impersonation ("Site admins can always login as someone else") and a one-click dashboard shortcut is a different risk profile from core's deliberate multi-step flow. The decision lives in `classes/loginas_guard.php` (page-free, unit-tested) rather than in `index.php`. CSRF comes from `moodleform`'s built-in sesskey handling; the control is hidden entirely from anyone without `moodle/user:loginas`, and while already impersonating someone. |
| Health signal: Expired enrolments (2026-09-24, released 2.5.0) | `user_enrolments.status = 0` (ENROL_USER_ACTIVE) paired with `timeend > 0 AND timeend < now` - both conventions verified against `lib/enrollib.php` rather than assumed (`enrol_plugin::enrol_user()` defaults `$timeend = 0` for "no end date", and core's own expiry-notification query uses the same `timeend > 0` idiom). Deleted/guest/remote accounts are excluded, matching the "active account" convention `duplicate_emails()` already uses. Drill-down links into each course's participants page, where an expired enrolment is actually extended or removed. |
| Health signal: Self-enrolment without key or end date (2026-09-24, released 2.5.0) | Enabled `enrol = 'self'` instances where the enrolment key is empty **or** `enrolenddate = 0` - an OR, since either weakness alone leaves a course taking enrolments unsupervised. SPEC section 11's one-line backlog note ("without a key/without an end date") did not pin this down, so the drill-down reports per-row `nokey`/`noenddate` flags: the admin sees which weakness actually applies instead of the ambiguity having to be resolved up front. Counts enrolment *instances*, not courses (a course can have several). Field semantics verified against `enrol/self/lib.php`. |
| Quick action: Purge ALL site caches (2026-08-02, released 2.2.0) | A second, more impactful button next to the existing per-plugin "Purge cache now" - calls core's `purge_all_caches()`, gated by `moodle/site:config` (stricter than the page's own `local/admincockpit:view`, matching what `admin/purgecaches.php` itself requires) plus `require_sesskey()`. Added reactively after adding new plugin classes revealed that Moodle's class-autoloader cache doesn't notice new files without either a version bump or a full cache purge. |

### Extension mechanism: why hooks, not a settings field or a subplugin relationship

Considered and rejected during design (2026-08-02):

- **A raw-SQL admin settings field** ("type a query, get a signal"): this is the "arbitrary SQL dashboard widget" already rejected once in section 10 as disproportionate effort/risk for v1 - still true here, and worse, since it would mean executing admin-supplied SQL strings on every dashboard render with real injection/DoS surface, for something hooks already solve with reviewable, versioned code.
- **A formal Moodle subplugin relationship** (`db/subplugins.json`): too tightly coupled - it would mean this plugin owns installing/upgrading every signal-contributing plugin, exactly the opposite of the loose coupling a health signal from an independently-maintained, independently-released plugin needs.
- **Hooks** were chosen because they match the actual persona this serves: a developer-capable institution (or plugin author) with bespoke or private logic that this plugin's maintainer will never own, write, or need to review - not an admin with "just an idea" and no PHP skills. That latter case (e.g. "courses with more than N participants") is better served by requesting it as a GitHub issue and, if generically useful, having it land as a first-party built-in signal (like "unpublished courses" above) with a settings toggle - no plugin-authoring needed from the requester at all.
